...
$(document).ready(function() {
  ...
  var table=$('#example').DataTable({
    ...
    function mostrarmodal(dato1,dato2){
		  ...
		  var modal = $(this);
		  modal.find('.modal-header h4').text('Modificar Masa: ' + dato1);
	
		  modal.find('.modal-body #idMasa').val(dato1);
    ...
    function verNuevo(){
    	...
    	limpiarfields();
    	modal.find('.modal-header h4').text('Registrar Masa: ');
    	modal.find('.modal-body #idMasa').val('0');
    	$("#delete").hide();
    };
	...
	$('#btnNuevo').click(function(){
	  ...
	  $('#delete').click(function(){
	      	$('#borrame').remove();
	      	var modal = $('#myModalNuevo');
	      	var id=modal.find('.modal-body #idMasa').val();
	      	var c1="<a id=\"borrame\" class=\"btn btn-primary \" ";
	      	var str1 = "href=\"deleteDough?masa.idMasa=";
	        var str2 = "\">Si</a>";
	  ...
	  $('#RegistroMasa').submit(function(event){
				  ...
				  //si no es valido no hacemos nada
					...
					$.ajax({
						...
						success:function(data){
							$.growl(
								{
									title:" <strong>Masa: "+data.masa.descripcion+"</strong> ",
...
<div class="modal-dialog">
  ...
  <s:form id="RegistroMasa" action="createDough" enctype="multipart/form-data" method="post" acceptcharset="utf-8" theme="bootstrap" cssClass="animate-form well form-vertical">
    <div class="modal-header">
    	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
    	<h4>Registrar Masa</h4>
    </div>
		<div class="modal-body">
	    		<s:hidden id="idMasa" name="masa.idMasa" />
	    	<div class="form-group">
		   		<s:textfield label="Masa :" name="masa.descripcion" id="descripcion" cssClass="form-control"/>
	    	</div>
